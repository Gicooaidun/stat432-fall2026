...
`netid` means the student's Illinois NetID. It is not a UIN, Canvas numeric ID,
email address, or GitHub username. The canonical `author` value is the NetID
alone. A student may optionally use `"Display name or GitHub handle (netid)"`
on the same `author` line; a real or full name is never required. Keep the YAML
frontmatter to the five lines shown above.

A valid file has:

- the same two-digit `NN` in the filename and `id`;
- required `id`, `title`, and `author`, with the NetID included in a stable `id` using only letters, numbers, hyphens, and underscores;
- an `author` equal to the NetID, or an optional display name or GitHub handle followed by the NetID in parentheses;
